  <head>
    <meta charset="utf-8">
    <title>Dashboard</title>
    <link rel="shortcut icon" type="image/x-icon" href="views/assets/image/logo.png">
    <script type="text/javascript" src="//code.jquery.com/jquery-1.12.4.js"></script>
    <script type="text/javascript" src="https://cdn.datatables.net/1.10.16/js/jquery.dataTables.min.js"></script>
    <link rel="stylesheet" href="https://cdn.datatables.net/1.10.16/css/jquery.dataTables.min.css">
  </head>
  <body class="body-dash">

<?php require_once 'views/include/scope.menutopdashboardegg.php';
      require_once 'views/include/scope.profileenterprisegg.php';
      require_once 'views/include/scope.navegg.php';?>
      <script type="text/javascript" src="views/assets/js/datagridpc.js"></script>
  </body>
